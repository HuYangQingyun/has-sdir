...
corpora. The vectoriser uses English stop-words and the tokeniser does not
segment scripts such as Chinese. Do not run it on non-Latin text; results
there are not meaningful. Multilingual handling is planned, not present.

KNOWN BOUNDARIES (this minimal version) — identified during development with
the help of independent reviewers. They mark where a lightweight, purely
statistical approach reaches its limit, not defects to apologise for: a
synthetic baseline can cancel the relative signals; light contamination is
under-detected (separating it from narrow human text needs heavier methods);
cosmetic edits can lower the surface signals. A robust engineering-grade
implementation (perplexity, semantic embeddings) is deliberately out of scope
and is what a direct collaboration would build. See README for detail.
