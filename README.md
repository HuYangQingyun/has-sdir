...
- A batch carrying a **moderate share of synthetic material** is flagged, and the
  reported contaminated fraction moves in step with the true proportion. (Exact
  detection ranges are deliberately left unquantified here: the precise operating
  characteristics belong to the underlying method, which is not published in this
  lightweight build.)
...
- **Multiple mixed sources (the two-population simplification).** This
  lightweight build reads a batch as, in effect, a native population and a
  foreign one. Real contamination may blend several different generators, each
  with its own signature, and those can blur the separation. This is **not
  peculiar to SDIR**: it is a shared limit of the whole lightweight, zero-shot
  statistical tier, and peer methods lose the same separation under multi-source
  noise. Cleanly resolving it means leaving this tier for a heavier,
  multi-prototype approach. A note on direction: the deeper method does not try
  to count or fingerprint sources at all. It does not care how many generators
  entered or what each looks like; it observes only the **fluctuation the
  combined input induces in the running system**. Observed that way, the number
  of sources stops being a parameter. That view belongs to the Engineering
  Edition, not to this static build.
...
- **Synthetic text that is highly similar to human text.** If synthetic and
  ...
  is more reliable. The same holds for legitimately narrow, templated **human**
  text in general: any highly formulaic register can sit close to this line, and
  lightweight statistical methods across the tier are known to raise false
  positives on such text. This build is deliberately **general, not
  domain-specific**: one field's normal structure is not another's, and a general
  read-out cannot encode every domain's rules without ceasing to run as a general
  tool. Where a particular domain matters, the right answer is a **calibrated
  layer built for that domain**, added on request, not the general version
  stretched to fit.
