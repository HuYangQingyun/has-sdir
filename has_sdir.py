...
KNOWN OPEN ISSUES (v1.1) — adversarial testing has shown v1.1 is not yet
robust. A synthetic baseline can cancel the relative signals (baseline
poisoning); gradual contamination (10 to 30 percent) still reads CLEAR;
cosmetic edits can zero the text-intrinsic signals. These are being addressed
by an architecture-level revision in the next version. See README for detail.
