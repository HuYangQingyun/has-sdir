[README.md](https://github.com/user-attachments/files/30726779/README.md)
# HAS-Core :: SDIR
...
>
> **Known open issues (being addressed in the next version).** Further
> adversarial testing has shown that v1.1 is not yet robust. The following are
> confirmed and reproducible, and should be understood before relying on any
> score:
>
> - **Baseline poisoning.** Because recursion and diversity are now measured
>   relative to the baseline, supplying an equally synthetic baseline cancels
>   both signals: a fully recursive batch can read 0.000 CLEAR. The reference
>   frame is currently under the caller's control and is not itself audited.
> - **Gradual contamination goes undetected.** A batch that is 10 to 30 percent
>   synthetic still reads CLEAR, and does not reach REVIEW until it is already
>   severe. This is the regime the tool most needs to catch, and currently does
>   not.
> - **Cosmetic evasion.** Rotating openers and injecting a few rare tokens
>   drives both text-intrinsic signals to zero while the recursive core is
>   unchanged.
> - **English only (current scope).** The vectoriser uses English stop-words
>   and the tokeniser does not segment scripts such as Chinese. This release is
>   scoped to English-language corpora; do not run it on non-Latin text, where
>   results are not meaningful. Multilingual handling is planned, not present.
>
> The next version focuses on **English-language corpora**; multilingual
> support (including Chinese) is on the roadmap, not in the current scope. Its
> priority is to close the three language-independent gaps above by separating
> structural measurement from the reference layer at the architecture level and
> giving the baseline the same provenance treatment as the batch, rather than
> patching them case by case. With sincere thanks to the reviewers who broke it.
...
demonstrates the one attack v1.1 does close; see **Known open issues** above for
the attacks it does not.
